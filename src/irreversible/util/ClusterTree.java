...
@Description("Create initial beast.tree by hierarchical clustering, either through one of the classic link methods " +
		...
		"<br/>o irreversible, distance between sets is calculated as follows: "
		+ "for every sequence in group 1 create a sequenc with A if any of the sequences has an A, G otherwise (i.e. all taxa hava mutated to G). "
		+ "same for group 2, then count number of mismatches as distance. Add to this the absolute difference of group heights. " +
        "<br/>o single link, " +
        "<br/>o complete link, " +
...
public class ClusterTree extends Tree implements StateNodeInitialiser {
    ...
    public enum Type {irreversible, single, average, complete, upgma, mean, centroid, ward, adjcomplete, neighborjoining, neighborjoining2}
        ...
        while (clusters > 1) {
            ...
            for (int i = 0; i < instances; i++) {
                if (i != min1 && clusterID[i].size() != 0) {
                    ...
                    double distance = getDistance(distance0, clusterID[i1], clusterID[i2]);
                    if (linkType == Type.irreversible) {
                    	distance += Math.abs((clusterNodes[i1] == null ? 0 : clusterNodes[i1].m_fHeight) - 
                    			             (clusterNodes[i2] == null ? 0 : clusterNodes[i2].m_fHeight));
                    }
    ...
    void merge(int min1, int min2, double dist1, double dist2, final List<Integer>[] clusterID, final NodeX[] clusterNodes) {
        ...
        if (distanceIsBranchLength) {
            ...
            if (node.m_left != null && node.m_fHeight < node.m_left.m_fHeight) {
            	node.m_fHeight = node.m_left.m_fHeight;
            }
            if (node.m_right != null && node.m_fHeight < node.m_right.m_fHeight) {
            	node.m_fHeight = node.m_right.m_fHeight;
            }
        }
    ...
    double getDistance(final double[][] distance, final List<Integer> cluster1, final List<Integer> cluster2) {
        ...
        switch (linkType) {
            ...
            case average:
                ...
                break;
            case irreversible:
            	char [] chars1 = getchars(cluster1);
            	char [] chars2 = getchars(cluster2);
    ...
    /** return sequence containing an A if any of the sequences in the cluster contains an A 
     *  it will only contain a G in a particular position when all taxa have a G in that postition 
     *  **/
    private char[] getchars(List<Integer> cluster1) {
    	String seq = dataInput.get().getSequenceAsString(taxaNames.get(cluster1.get(0)));
    	char [] chars = new char[seq.length()];
    	for (int i = 0; i < chars.length; i++) {
    		chars[i] = seq.charAt(i);
    	}
        for (int i = 1; i < cluster1.size(); i++) {
        	String seq2 = dataInput.get().getSequenceAsString(taxaNames.get(cluster1.get(i)));
